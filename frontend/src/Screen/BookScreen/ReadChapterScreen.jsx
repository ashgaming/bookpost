...
export default function ReadChapterScreen() {
  ...
  const chapter = {
    ...
    likes: 5,
    rating: 4,
    url: 'https://via.placeholder.com/150', // Example URL for chapter image
  };
  ...
  useEffect(() => {
    setStory(`At midnight the cafe was crowded. By some chance the little table at which I sat had escaped the eye of incomers, and two vacant chairs at it extended their arms with venal hospitality to the influx of patrons.
      ...
      <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-6">
        ...
        <section className="mb-8">
          <div className="flex flex-col md:flex-row items-center mb-6">
            <img src={chapter.url} alt="Chapter Image" className="w-48 h-48 object-cover rounded-lg shadow-md" />
            <div className="mt-4 md:mt-0 md:ml-6">
              <h1 className="text-2xl font-bold text-gray-800">{title || 'Untitled'}</h1>
              <h2 className="text-lg text-gray-600">{chapter.name}</h2>
              <Stars rating={3} count={4} />
        ...
        <div className="flex justify-between mb-6">
          {chapterid > 1 && (
            <Link
              to={`/story/${storyid}/chapter/${chapter.chapterid - 1}`}
              className="text-blue-500 hover:text-blue-700 font-semibold"
            >
              Previous
            </Link>
          )}
          {chapter.totalchapters !== parseInt(chapterid) && (
            <Link
              to={`/story/${storyid}/chapter/${parseInt(chapter.chapterid) + 1}`}
              className="text-blue-500 hover:text-blue-700 font-semibold"
            >
              Next
            </Link>
